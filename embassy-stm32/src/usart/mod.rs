...
impl<'d> UartTx<'d, Async> {
    ...
    pub fn new_with_cts<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        tx: impl Peripheral<P = impl TxPin<T>> + 'd,
...
impl<'d> UartTx<'d, Blocking> {
    ...
    ///
    /// Useful if you only want Uart Tx. It saves 1 pin and consumes a little less power.
    pub fn new_blocking<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        tx: impl Peripheral<P = impl TxPin<T>> + 'd,
        config: Config,
    ) -> Result<Self, ConfigError> {
    ...
    pub fn new_blocking_with_cts<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        tx: impl Peripheral<P = impl TxPin<T>> + 'd,
        cts: impl Peripheral<P = impl CtsPin<T>> + 'd,
        config: Config,
...
impl<'d> UartRx<'d, Async> {
    ...
    pub fn new_with_rts<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        _irq: impl interrupt::typelevel::Binding<T::Interrupt, InterruptHandler<T>> + 'd,
        rx: impl Peripheral<P = impl RxPin<T>> + 'd,
...
impl<'d> UartRx<'d, Blocking> {
    ...
    /// Create a new rx-only UART with a request-to-send pin
    pub fn new_blocking_with_rts<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        rx: impl Peripheral<P = impl RxPin<T>> + 'd,
        rts: impl Peripheral<P = impl RtsPin<T>> + 'd,
        config: Config,
...
impl<'d> Uart<'d, Async> {
    ...
    pub fn new_with_rtscts<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        rx: impl Peripheral<P = impl RxPin<T>> + 'd,
        tx: impl Peripheral<P = impl TxPin<T>> + 'd,
        _irq: impl interrupt::typelevel::Binding<T::Interrupt, InterruptHandler<T>> + 'd,
    ...
    pub fn new_with_de<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        rx: impl Peripheral<P = impl RxPin<T>> + 'd,
        tx: impl Peripheral<P = impl TxPin<T>> + 'd,
        _irq: impl interrupt::typelevel::Binding<T::Interrupt, InterruptHandler<T>> + 'd,
    ...
    #[doc(alias("HDSEL"))]
    pub fn new_half_duplex<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        tx: impl Peripheral<P = impl TxPin<T>> + 'd,
        _irq: impl interrupt::typelevel::Binding<T::Interrupt, InterruptHandler<T>> + 'd,
        tx_dma: impl Peripheral<P = impl TxDma<T>> + 'd,
    ...
    #[cfg(not(any(usart_v1, usart_v2)))]
    #[doc(alias("HDSEL"))]
    pub fn new_half_duplex_on_rx<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        rx: impl Peripheral<P = impl RxPin<T>> + 'd,
        _irq: impl interrupt::typelevel::Binding<T::Interrupt, InterruptHandler<T>> + 'd,
        tx_dma: impl Peripheral<P = impl TxDma<T>> + 'd,
...
impl<'d> Uart<'d, Blocking> {
    ...
    /// Create a new bidirectional UART with request-to-send and clear-to-send pins
    pub fn new_blocking_with_rtscts<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        rx: impl Peripheral<P = impl RxPin<T>> + 'd,
        tx: impl Peripheral<P = impl TxPin<T>> + 'd,
        rts: impl Peripheral<P = impl RtsPin<T>> + 'd,
    ...
    #[cfg(not(any(usart_v1, usart_v2)))]
    /// Create a new bidirectional UART with a driver-enable pin
    pub fn new_blocking_with_de<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        rx: impl Peripheral<P = impl RxPin<T>> + 'd,
        tx: impl Peripheral<P = impl TxPin<T>> + 'd,
        de: impl Peripheral<P = impl DePin<T>> + 'd,
    ...
    /// on the line must be managed by software (for instance by using a centralized arbiter).
    #[doc(alias("HDSEL"))]
    pub fn new_blocking_half_duplex<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        tx: impl Peripheral<P = impl TxPin<T>> + 'd,
        mut config: Config,
    ) -> Result<Self, ConfigError> {
    ...
    #[cfg(not(any(usart_v1, usart_v2)))]
    #[doc(alias("HDSEL"))]
    pub fn new_blocking_half_duplex_on_rx<T: BasicInstance>(
        peri: impl Peripheral<P = T> + 'd,
        rx: impl Peripheral<P = impl RxPin<T>> + 'd,
        mut config: Config,
    ) -> Result<Self, ConfigError> {
...
/// Info about a concrete USART peripheral instance.
///
...
struct Vtable {
    ...
    disable_fn: fn(),
    interrupt_enable_fn: unsafe fn(),
    interrupt_disable_fn: fn(),
    interrupt_pend_fn: fn(),
    interrupt_unpend_fn: fn(),
}
        ...
        impl SealedBasicInstance for crate::peripherals::$inst {
            fn vtable() -> &'static Vtable {
                static VTABLE: Vtable = Vtable {
                    ...
                    disable_fn: <crate::peripherals::$inst as crate::rcc::SealedRccPeripheral>::disable,
                    interrupt_enable_fn: crate::interrupt::typelevel::$irq::enable,
                    interrupt_disable_fn: crate::interrupt::typelevel::$irq::disable,
